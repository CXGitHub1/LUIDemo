...
function ModelLoader:Load(modelId, skinId, animationId)
    ...
    local result = GameObject.Instantiate(prefab)
    -- local animation = result:GetComponent(Animation)
    local render = result:Find("Mesh_body"):GetComponent(Renderer)
    render.material = AssetLoader.Instance:Load(string.format(AssetDefine.SKIN_PATH, modelId))
    return result
end



CacheNpcTpose = CacheNpcTpose or BaseClass(GoPoolObject)

local _Time = Time
local _string_format = string.format

function CacheNpcTpose:__init()
    self.is_cache = true
    self.animation = nil
    self.resData = nil
    self.texture_id = nil
...
function CacheNpcTpose:__delete()
    self.is_cache = true
    self.animation = nil
    self.resData = nil
    self.texture_id = nil
...
function CacheNpcTpose:Reset()
    self.time = _Time.time
    local parent = GoPoolManager.Instance.npcTransform
    self:RemoveMountEffect()
    self:ResetModel(parent)
    self:GetAnimation().enabled = false
