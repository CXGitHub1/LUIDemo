...
function LScrollPageTest:__init(gameObject)
    ...
    -- local scrollPage = FSScrollPage.New(transform:Find("Test0"), LTestItem)
    -- scrollPage:SetData()
    -- local scrollPage = FSScrollPage.New(transform:Find("Test01"), LTestItem)
    -- scrollPage:SetData({})
    for i = 1, #LScrollPageTest.Config do
        local config = LScrollPageTest.Config[i]
        local scrollPage = FSScrollPage.New(transform:Find("Test" .. i), LTestItem, config.row, config.column, config.direction)
